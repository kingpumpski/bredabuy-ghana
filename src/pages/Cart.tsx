    ...
    <>
      ...
      <main className="container mx-auto px-4 py-8 md:py-12" aria-labelledby="cart-title">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <aside className="lg:col-span-1" aria-label="Order summary"><div className="sticky top-24 rounded-2xl border border-border bg-card p-6 shadow-sm"><h2 className="mb-6 text-xl font-display font-bold text-foreground">Order Summary</h2><div className="mb-6 space-y-4"><div className="flex justify-between gap-4 text-sm"><span className="text-muted-foreground">Subtotal ({totalItems} items)</span><span className="font-medium">{formatPrice(subtotal)}</span></div><div className="flex justify-between gap-4 text-sm"><span className="text-muted-foreground">Shipping</span><span className="font-medium text-secondary">{shipping === 0 ? "Free" : formatPrice(shipping)}</span></div><div className="flex justify-between gap-4 text-sm"><span className="text-muted-foreground">VAT</span><span className="font-medium">{formatPrice(vat)}</span></div><div className="h-px bg-border" /><div className="flex justify-between gap-4 text-lg font-bold"><span>Total</span><span className="text-primary">{formatPrice(total)}</span></div></div>{subtotal < FREE_SHIPPING_THRESHOLD && <p className="mb-4 rounded-lg bg-muted/50 p-3 text-sm text-muted-foreground">Add {formatPrice(FREE_SHIPPING_THRESHOLD - subtotal)} more for free shipping.</p>}<Link to="/checkout" aria-disabled={hasUnavailableItems} className={hasUnavailableItems ? "pointer-events-none" : ""}><Button variant="hero" className="w-full" disabled={hasUnavailableItems}>Proceed to Checkout<ArrowRight className="ml-2 h-5 w-5" aria-hidden="true" /></Button></Link><Link to="/products" className="mt-4 block"><Button variant="outline" className="w-full">Continue Shopping</Button></Link><div className="mt-6 border-t border-border pt-6"><p className="mb-3 text-sm text-muted-foreground">Available payment methods:</p><div className="flex flex-wrap gap-2">{["Mobile Money", "Bank Transfer", "Cash on Delivery"].map((method) => <span key={method} className="rounded-full bg-muted px-3 py-1 text-xs font-medium">{method}</span>)}</div></div></div></aside>
        </div>
      </main>
    </>
  );
