...
const makeCombinations = (groups: VariantOptionGroup[]) => {
  ...
  return active.reduce<Record<string, string>[]>((combinations, group) => {
    ...
    if (!combinations.length) {
      return values.map((value) => ({ [group.name.trim()]: value }));
    }
...
export default function VariantManager({ value = [], onChange }: VariantManagerProps) {
  ...
  const updateValue = (groupIndex: number, valueIndex: number, value: string) => {
    setGroups((current) =>
      current.map((group, index) => {
        if (index !== groupIndex) return group;
        return {
          ...group,
          values: group.values.map((item, i) => (i === valueIndex ? value : item)),
        };
      }),
    );
  };
            ...
            <div key={`${group.name}-${groupIndex}`} className="rounded-xl border p-4">
              <div className="flex items-center gap-2">
                <Input
                  ...
                  placeholder="Option name e.g. Size, Colour, Material"
                    ...
                    <tr key={variant.id} className="border-t align-middle">
                      ...
                      <td className="p-3">
                        <Input
                          ...
                          min={0}
                          value={variant.price}
                          onChange={(event) => updateVariant(variant.id, { price: Number(event.target.value) || 0 })}
                        />
                      </td>
                      <td className="p-3">
                        <Input
                          ...
                          min={0}
                          value={variant.stock}
                          onChange={(event) => updateVariant(variant.id, { stock: Number(event.target.value) || 0 })}
                        />
                      </td>
                      <td className="p-3">
                      ...
                      <td className="p-3 text-right">
                        <Button type="button" size="icon" variant="ghost" onClick={() => removeVariant(variant.id)} aria-label="Remove variant">
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </td>
                    </tr>
