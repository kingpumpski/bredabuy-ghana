...
export const refundService={
 ...
 request(input:{inspectionId:string;destination?:RefundDestination}){const inspection=returnInspectionService.list().find(x=>x.id===input.inspectionId);if(!inspection||!inspection.refundEligible||!inspection.refundAmount)return null;const request=returnInspectionService;const returnRecord=returnShipmentServicePlaceholder(inspection.returnId);if(!returnRecord)return null;const now=new Date().toISOString();const refund:Refund={id:crypto.randomUUID(),orderId:returnRecord.orderId,returnId:returnRecord.id,amount:inspection.refundAmount,currency:"GHS",destination:input.destination??"wallet",status:"requested",reason:"Approved return inspection",createdAt:now,updatedAt:now};write([refund,...read()]);return refund;},
...
function returnShipmentServicePlaceholder(id:string){
  try { const raw=localStorage.getItem("bredabuy:reverse-logistics"); const items=raw?JSON.parse(raw) as Array<{id:string;orderId:string}>:[]; return items.find(x=>x.id===id)??null; } catch { return null; }
}
